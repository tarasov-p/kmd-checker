...
## Архив проверок (персистентный лог)

Сессии живут в памяти и чистятся по TTL, но каждая **завершённая** проверка
дополнительно складывается на диск — переживает рестарт контейнера:

```
{KMD_ARCHIVE_DIR}/
├── metrics.jsonl                       # по строке на проверку (append, удобно грепать)
└── 2026-05-29/
    └── <session_id>/
        ├── <исходный файл>             # то, что загрузил пользователь
        └── result.json                 # полный ответ: verdict, findings, reasoning, cost…
```

- `KMD_ARCHIVE_DIR` — каталог архива (в Docker bind-mount `./data/archive:/archive`).
- `KMD_ARCHIVE_ENABLED=0` — выключить архивирование.
- Архив не чистится автоматически — следите за местом на диске.
- Сумма стоимости за период: `jq .cost_usd data/archive/metrics.jsonl | paste -sd+ | bc`.
